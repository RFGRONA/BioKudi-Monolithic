...
namespace BioKudi.Services
{
    public class MapService
    {
        ...
        .swal2-cancel {
            ...
            {
                html.Append("<p>No hay reseñas todavía.<br>¡Sé el primero en dejar una!</p>");
            }
        ...
        <script>
            ...
            html.Append($"showReviews(placeId,{WebUtility.HtmlEncode(idUser.ToString())})");
            html.Append(@"},
                            error: function (err) {
    ...
    ");
        ...
        public ContentResult CreateReviewForm(int idPlace, HttpContext httpContext)
        {
            var userIdClaim = httpContext.User.FindFirst(ClaimTypes.NameIdentifier);
            int idUser = 0;
            if (userIdClaim != null)
    ...
    ";
        ...
        <script>
            $(document).off('submit', '#createReviewForm').on('click', '#createReviewForm input[type=""submit""]', function (event) {
                event.preventDefault();
                var form = $(this).closest('form');
                Swal.fire({
                    title: '¿Estás seguro?',
                    text: 'Estás a punto de enviar tu reseña.',
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                    ...
                    if (result.isConfirmed) {
                        ...
                        var placeId = form.find('input[name=""PlaceId""]').val();
    ...
    ");
        ...
        public ContentResult UpdateReviewForm(int idReview, HttpContext httpContext)
        {
            var userIdClaim = httpContext.User.FindFirst(ClaimTypes.NameIdentifier);
            int idUser = 0;
            if (userIdClaim != null)
...
";
        ...
        <script src='https://code.jquery.com/jquery-3.6.0.min.js'></script>
        <script>
            $(document).off('submit', '#updateReviewForm').on('click', '#updateReviewForm input[type=""submit""]', function (event) {
                event.preventDefault();
                var form = $(this).closest('form');
                Swal.fire({
                    title: '¿Estás seguro?',
                    text: 'Estás a punto de actualizar tu reseña.',
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
